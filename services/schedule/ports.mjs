...
// ---------------------------------------------------------------------------
export function createInMemoryIdentity({ memberships = [], projects = [] } = {}) {
  // memberships: [{ projectId, partyId, role }]
  // projects: [{ projectId, name }] — the RFP send path dereferences the project
  // name through the identity seam (ADR-0006); tests that exercise it supply a
  // name, everyone else gets a null-safe default.
  const byProject = new Map();
  for (const m of memberships) {
    if (!byProject.has(m.projectId)) byProject.set(m.projectId, new Map());
    byProject.get(m.projectId).set(m.partyId, m.role);
  }
  const byProjectName = new Map(projects.map((p) => [p.projectId, p.name]));

  function roleOf(projectId, partyId) {
    return byProject.get(projectId)?.get(partyId) ?? null;
  }

  // The RFP send path reads the project NAME for the invitation subject. It
  // requires the VIEW_PROJECT capability (both RFP managers hold it), mirroring
  // the identity service's getProject. Returns only the name the email needs —
  // never a raw store row.
  async function getProject({ actorPartyId, projectId }) {
    authorize({ actorPartyId, action: ACTION.VIEW_PROJECT, projectId });
    return { id: projectId, name: byProjectName.get(projectId) ?? null };
  }

  // The RFP token page's party-less project preview — mirrors the identity
  // service's getProjectPreview (same seam, same reserved-null `location`). No
  // authorization: the RFP service calls it only after ITS store resolved a real
  // token hash, so a forged projectId never gets here.
  async function getProjectPreview({ projectId }) {
    const name = byProjectName.get(projectId) ?? null;
    if (name == null) return { id: null, name: null, location: null };
    return { id: projectId, name, location: null };
  }
  ...
  return { authorize, requireMember, roleOf, getProject, getProjectPreview };
}
...
export function createInMemoryStore() {
  ...
  // LINA-277/279 (ADR-0023): project phase + RFP procurement loop state.
  const phases = new Map();    // project_phase rows (keyed by id)
  const rfps = [];             // schedule.rfp rows
  const recipients = [];       // schedule.rfp_recipient rows
  const proposals = [];        // schedule.rfp_proposal rows (append-only)
  ...
  // In-memory mirrors of the six migration-0012 tables. Phases are seeded by the
  // schedule service builder for the projects under test; rfps/recipients/
  // proposals are plain arrays (the primary key is generated by the caller, so no
  // identity column bookkeeping is needed).

  function getPhase(id) {
    if (id == null) return null;
    const r = phases.get(id);
    return r ? { ...r } : null;
  }

  function getPhaseByProjectAndKind(projectId, kind) {
    for (const r of phases.values()) {
      if (r.project_id === projectId && r.kind === kind) return { ...r };
    }
    return null;
  }

  function insertRfp(_tx, row) {
    const stored = { ...row };
    rfps.push(stored);
    return { ...stored };
  }
  ...
  function getRfp(id) {
    if (id == null) return null;
    const r = rfps.find((x) => x.id === id);
    return r ? { ...r } : null;
  }

  function getLiveRfpByPhase(phaseId) {
    const r = rfps
      .filter((x) => x.phase_id === phaseId && x.status !== 'closed')
      .sort((a, b) => (a.created_at < b.created_at ? 1 : -1))[0];
    return r ? { ...r } : null;
  }

  function updateRfp(_tx, id, patch) {
    const r = rfps.find((x) => x.id === id);
    if (!r) return null;
    if (patch.description !== undefined) r.description = patch.description;
    if (patch.attachments !== undefined) r.attachments = patch.attachments;
    if (patch.specialties !== undefined) r.specialties = patch.specialties;
    if (patch.status !== undefined) r.status = patch.status;
    r.updated_at = now();
    return { ...r };
  }

  function insertRfpRecipient(_tx, row) {
    const stored = { ...row };
  ...
  // to the schedule_app exactly for this); a sent RFP's recipients hold live
  // tokens, so the service refuses it then — the store just deletes.
  function deleteRfpRecipient(_tx, id) {
    const i = recipients.findIndex((x) => x.id === id);
    if (i < 0) return null;
    const [removed] = recipients.splice(i, 1);
    return { ...removed };
  }

  function listRfpRecipients(rfpId) {
    return recipients
      .filter((x) => x.rfp_id === rfpId)
      .sort((a, b) => (a.created_at < b.created_at ? -1 : a.created_at > b.created_at ? 1 : 0))
      .map((x) => ({ ...x }));
  }

  function getRfpRecipientByTokenHash(tokenHash) {
    const rec = recipients.find((x) => x.token_hash === tokenHash);
    if (!rec) return null;
    const rfp = rfps.find((x) => x.id === rec.rfp_id);
    const phase = rfp ? phases.get(rfp.phase_id) : null;
    if (!rfp || !phase) return null;
    return {
      recipient: { ...rec },
      rfp: { ...rfp },
      phase: { ...phase },
  ...
  function updateRfpRecipientStatus(_tx, id, status) {
  ...
  // so the deliverable link exists only in the email; ADR-0023 §5). Mirrors the
  // pg adapter's UPDATE — only token_hash moves, never the identity.
  function rotateRfpRecipientToken(_tx, id, tokenHash) {
    const r = recipients.find((x) => x.id === id);
    if (!r) return null;
  ...
  function insertRfpProposal(_tx, row) {
    const stored = { ...row };
    proposals.push(stored);
    return { ...stored };
  }

  function listRfpProposals(rfpId) {
    return proposals
      .filter((x) => x.rfp_id === rfpId)
      .sort((a, b) => (a.submitted_at < b.submitted_at ? 1 : -1))
      .map((x) => {
        const rec = recipients.find((r) => r.id === x.rfp_recipient_id);
        // pg adapter joins the recipient's public email face; mirror it so both
        // adapters shape the inbox identically.
        return { ...x, recipient_email: rec?.email ?? null, recipient_status: rec?.status ?? null };
      });
  }
  ...
  return {
    ...
    _phases: phases, _rfps: rfps, _recipients: recipients, _proposals: proposals,
  };
}
