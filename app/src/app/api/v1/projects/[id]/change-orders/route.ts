...
export function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  return handle(req, ctx, (c) => container().http.changeOrder.listChangeOrders(withProjectId(c)));
}

export function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  return handle(req, ctx, (c) => container().http.changeOrder.proposeChangeOrder(withProjectId(c)));
}
