...
console.log('endpoints');
{
  ...
  eq('p=0 → bars/HUD on', a.chromeOpacity, 1);
  ...
  // LINA-117, the founder's note: "the first image should already have the blue
  // (plan) bars showing the original plan". The pen agrees — KF A draws all six
  // baselines at full width — so p=0 holds KF A's geometry rather than growing
  // into it.
  check(
    'p=0 → every planned bar already at its KF A width',
    SEQUENCE_ROW_KEYS.every((k) => a.rows[k].plannedWidth === SEQUENCE_ROW_GEOMETRY[k].plannedWidth)
  );
...
// --- The chrome matches the pen ---------------------------------------------

console.log('chrome (pen: Caption, Gantt and HUD at full opacity in every KF)');
{
  for (const p of [0, ...SEQUENCE_KEYFRAMES.map((kf) => kf.p), 0.275]) {
    eq(`bars/HUD on at p=${p}`, sequenceStateAt(p).chromeOpacity, 1);
  }
}
