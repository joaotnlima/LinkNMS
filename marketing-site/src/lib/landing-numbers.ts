...
/** Contract total after every signed change. Appears in S3, S4, S5, the HUD and the phone preview. */
...
/**
 * The pinned scroll sequence (`key-frames`).
 *
 * Four rows on a 320-unit track with 7-unit lanes, planned above actual. The
 * geometry per keyframe is the KF A–D table from the spec, wired literally so
 * the animation reads the table rather than eyeballing timings. `p` is recorded
 * here for the keyframe *boundaries* only — the visible stage is derived from
 * bar state, never from `p`.
 */
...
/** Native pixel size of the stage sources. Do not upscale past this (technical plan D2). */
export const SEQUENCE_IMAGE_WIDTH = 1402;
export const SEQUENCE_IMAGE_HEIGHT = 1122;

/** Phone preview in the CTA section. */
